...
for it in range(nt-1):
    ...
    if order<2:
        ...
        "Centered differences"
        Advp = -0.5*c*(1.5*np.diag(np.ones(N+2)) - 2*np.diag(np.ones(N+1),-1) + 0.5*np.diag(np.ones(N),-2))
        Advm = -0.5*c*(1.5*np.diag(np.ones(N+2)) - 2*np.diag(np.ones(N+1),1) + 0.5*np.diag(np.ones(N),2))
    ...
    "Boundary condition at x=1"
    A[N+1,:] = (1/Dx**2)*np.concatenate((np.zeros(N) +  ))
    F[N+1] = u0
